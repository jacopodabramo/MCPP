...
from utils import saving_file, format_output_mip_model1, format_output_mip_model0
from pulp import *
import numpy as np
import traceback
import time
...
class MIPsolver:
    ...
    def set_constraints_model1(self, data):
        ...
        # General variables
        asg = [[[LpVariable(name=f'asg_{k}-{i}-{j}', cat=LpBinary) for i in range(items)] for j in range(items)] for k
               in
               range(couriers)]

        weigths = [LpVariable(name=f'weigth_{i}', lowBound=0, upBound=courier_size[i], cat=LpInteger) for
                   i in range(couriers)]

        # Distances computation variables
        dist1 = [
            LpVariable(name=f'or_to_first_{i}', lowBound=0, upBound=np.max(distances), cat=LpInteger) for
            i in range(couriers)]

        dist2 = [LpVariable(name=f'last_to_or_{i}', lowBound=0, upBound=np.max(distances), cat=LpInteger)
                 for i in range(couriers)]

        dist2_bool = [[LpVariable(name=f'dist2_bool_{k}-{i}', cat=LpBinary) for i in range(items)] for k in
                      range(couriers)]
        ...
        for k in range(couriers):
            ...
            self.model += weigths[k] == LpAffineExpression(
                [(asg[k][i][j], item_size[j]) for i in range(items) for j in range(items)])
        ...
        for k in range(couriers):
            ...
            self.model += dist1[k] == LpAffineExpression([(asg[k][0][i], distances[items][i]) for i in range(items)])

        # 2 ) Last distribution center to Origin distance
        for k in range(couriers):
            self.model += lpSum(dist2_bool[k]) <= 1
            # self.model += lpSum(asg[k][i][j] for i in range(items) for j in range(items)) <= lpSum(dist2_bool[k]) * 1000
            for i in range(items - 1):
        ...
        for k in range(couriers):
            ...
            self.model += LpAffineExpression([
                ...
                for i in range(items) for j in range(items)]) == dist2[k]
        ...
        end_time = time.time()
        print("total time for setting = ", end_time - start_time)
    ...
    def set_constraints_model0(self, instance):
        ...
        for k in range(couriers):
            self.model += starting_point[k][0][items] == lpSum(ending_point[k][i][items] for i in range(items + 1))
